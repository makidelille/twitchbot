    ...
    protected void onMessage(String channel, String sender, String login, String hostname, String msg) {
        ...
        if (stream.isUserOp(sender)) {
            switch (cmd) {
                ...
                case "!alwaysanswer":
                    if (alwaysAnswer) {
                        sendMeText(channel, "je reponds deja √† tout :D", sender, TwitchColor.DARKPINK);
                        return;
                    }
                ...
                case "!botspam":
                    if (singleCmd) {
                        antiSpam = true;
                        sendMeText(channel, "AntiSpam actif avec pour d√©lai : " + spamDelay + "s", sender, TwitchColor.RED);
                        return;
                    }
                    try {
                        if (msgArray[1].equalsIgnoreCase("off")) {
                            antiSpam = false;
                            sendMeText(channel, "AntiSpam √©teint.Attention le chaos arrive...", sender, TwitchColor.GREEN);
                            return;
                        ...
                        sendText(channel, "[@s] : erreur dans les arguments, y a pas de d√©bat possible :o", sender);
                    }
                    return;
                case "!addmaster":
                    if (stream.isUserMaster(sender)) {
        ...
        if (mode.contains("+o")) {
            try {
                ...
                if (!stream.getUsers().getOpUsers().contains(modo)) {
                    Main.log(modo + " a ÈtÈ ajouter ‡ la liste des modos du stream de " + stream.getName());
            ...
            try {
                String modo = mode.split(" ")[2];
                if (stream.getUsers().getOpUsers().contains(modo)) {
                    Main.log(modo + " a ÈtÈ supprimer de la liste des modos du stream de " + stream.getName());
        ...
        if (tempStream != null) {
            if (tempStream.isChannelCmd(cmd)) {
                ...
                try {
                    ...
                    if (success) {
                        sendText(getStreamChannel(), "commande ajout√©e", sender);
                        stream = StreamerData.getStreamerData(channel);
                ...
                try {
                    ...
                    if (success) {
                        sendText(getStreamChannel(), "commande supprim√©e", sender);
                        stream = StreamerData.getStreamerData(channel);
